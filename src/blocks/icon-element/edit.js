...
export default function Edit({ attributes, setAttributes }) {    
    ...
    if (attributes.type === "font" && !attributes.style.icon) blockProps.className += ` fas fa-icons`;
        ...
        <BlockControls>
            <ToolbarGroup key={`${uniqueKey}-toolbar`} label={__("Quick Settings", "pds-blocks")}>
                ...
                {attributes.type === "font" ? 
                    <PxDropDown title={__("Change Icon", "pds-blocks")} button={`bg-transparent fs-16 square ${attributes.style?.icon || "fas fa-icons"} divider-e border-alpha-25 h-100`} dropList="fs-14 w-min-280">
                        <li key="icons" className='pd-10'>
                            {/*===> Icon Selector <===*/}
                            <PhenixIcons key={`icon-${uniqueKey}`} value={ attributes.style?.icon || "fas fa-icons" } onChange={set_icon} />
                            {/*===> Option Control <===*/}
                    ...
                    :
                    <PxDropDown title={__("Change Icon", "pds-blocks")} button={`bg-transparent fs-16 square fas fa-image divider-e border-alpha-25 h-100`} dropList="fs-14 w-min-280">
                        <li key="icons" className='pd-10'>
                            {/*===> Icon Uploader <===*/}
                ...
                {/*===> Link Input <===*/}
                {attributes.style.isLink ? <PxDropDown title={__("URL Options", "pds-blocks")} button={`bg-transparent fs-16 square fas fa-link color-success divider-e border-alpha-25 h-100`} dropList="fs-14 w-min-260" >
                ...
                <PxDropDown title={__("Style Options", "pds-blocks")} button={`bg-transparent fs-16 square fas fa-adjust divider-e border-alpha-25 h-100`} dropList="fs-14 w-min-280">
                    <li key="pds-styles" className='pdt-15 pdb-15 pdx-15 lineheight-150'>
                        {/*===> Icon Size <===*/}
                        <PhenixNumber name={"dimensions"} icon="fas fa-arrows" className="mb-15" min={0} max={1920} value={attributes.style.dimensions || 42} onChange={set_style} />
