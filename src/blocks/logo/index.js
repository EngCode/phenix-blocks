...
registerBlockType(metadata, {
    ...
    save : ({ attributes }) => {
        ...
        return (
            <a { ...blockProps }>
                <img src={attributes.logo} className={attributes.responsive ? 'hidden-md-down' : ''} alt={blockProps.title} style={{ "maxHeight": `${Phenix(document).toREM(attributes.size)}` }} width={Phenix(document).toREM(attributes.width) ? Phenix(document).toREM(attributes.width) : null} height={Phenix(document).toREM(attributes.size)} />
                {attributes.responsive ?  <img src={attributes.use_fevicon ? attributes.fevicon : attributes.mobile_logo} className='hidden-lg-up' alt={blockProps.title} width={Phenix(document).toREM(attributes.size)} height={Phenix(document).toREM(attributes.size)} /> : null}
            </a>
        );
    }
});