...
registerBlockType(metadata, {
    ...
    save : ({ attributes }) => {
        ...
        Object.entries(attributes).forEach(([option_name, option_value]) => {
            //===> Exclude Options <===//
            const excluded = ["tagName", "className", "align", "content", "lightbox_type"];
            ...
            if (isNormalValue(option_value) && attributes[option_name]) {
                ...
                //===> Other Options <===//
                else blockProps.className += ` ${attributes[option_name].toString().replace(',', ' ').trim()}`;
            }

            else if (isBooleanVal(option_value) && attributes[option_name]) {
                //===> for Lightbox Options <===//
                if (option_name === "isLightBox" && option_value) {
